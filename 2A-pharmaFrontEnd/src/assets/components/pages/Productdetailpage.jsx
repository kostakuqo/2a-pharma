...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true);
      try {
        // Încarcă produsul după ID Firestore
        ...
        // Încarcă produse related (aceeași categorie)
        const snap = await getDocs(collection(db, "products"));
        const all = snap.docs.map(d => ({ id: d.id, ...d.data() }));
        const rel = all.filter(r => r.category_en === p.category_en && r.id !== p.id).slice(0, 3);
        setRelated(rel);
